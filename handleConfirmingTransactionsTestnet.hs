...
program = do
  homedir <- pure chomp <*> readProcess "pwd" [] "" 
  let configFile = homedir </> "dotfiles/.bitcoin/bitcointestnet.conf"
  forever $ do
      let confirmingTransactionsFile = "confirmingTransactionsTestnet.txt"
      txids <- (pure lines) <*> (readFile confirmingTransactionsFile)
      forM_ txids $ \txid -> handleTransaction configFile txid
...
handleTransaction :: FilePath -> String -> IO ()
handleTransaction configFile txid = do
                  ...
                  then do
                    ...
                    case mbBalance of
                      ...
                      Just strBalance -> do
                        ...
                        (csvFile :: String) <- pure (</> "exchange_transactions.csv") <*> getHomeDirectory
                        putStrLn $ "about to write csv: " ++ csvFile
                        let csvLine = ( concat . intersperse "," $    
                              [ userId,
                                ...
                                show txoutIndex ] ) ++ "\n"
                        putStrLn csvLine
                        appendFile csvFile csvLine
                  else putStrLn $ show (txOutType, addresses)

bitcoinCli :: String -> [String] -> IO (Maybe String)
bitcoinCli configFile args = do
  ( exitCode, stdout, stderr) <- readProcessWithExitCode "bitcoin-cli" ([ "-conf=" ++ configFile] ++ args) "" -- make it readProcessWithExitCode
  case exitCode of
    ExitFailure code -> do
...
t1 = handleTransaction ( "/home/thomas/levidge-bitcoin-watching-wallet/dotfiles/.bitcoin/bitcointestnet.conf" ) "58dea6e73f4f1d6fa4fe499bc497de352ac9faf95c9dd6b0efe43a19765e741f"
t2 = handleTransaction ( "/home/thomas/levidge-bitcoin-watching-wallet/dotfiles/.bitcoin/bitcointestnet.conf" ) "dee6e5ea348b370081e7c86d1c09a958546cfd51188ec969dd1c31cf9d7b0218"
